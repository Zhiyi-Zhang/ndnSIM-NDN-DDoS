...
main(int argc, char* argv[]) {
  ...
  consumerHelper.SetAttribute("GoodConsumer", BooleanValue(false));
  consumerHelper.SetAttribute("GoodTrafficAlso", IntegerValue(20));
  ...
  Ptr<Node> as3_cs_server = Names::Find<Node>("as3-math-server");
  producerHelper.SetPrefix("/good");
   producerHelper.SetAttribute("ValidThreshold", StringValue("10000"));
  producerHelper.Install(as3_cs_server);

  // Installing global routing interface on all nodes
  ndn::GlobalRoutingHelper ndnGlobalRoutingHelper;
  ndnGlobalRoutingHelper.InstallAll();
  ndnGlobalRoutingHelper.AddOrigins("/u1", as1_cs_server);
  ndnGlobalRoutingHelper.AddOrigins("/good", as3_cs_server);
  ndnGlobalRoutingHelper.CalculateRoutes();

  Simulator::Stop(Seconds(20.0));
  ndn::L3RateTracer::InstallAll("src/ndnSIM/Results/fine-granularity/" + outFile + ".txt",
                                Seconds(0.3));
  Simulator::Run();
  Simulator::Destroy();
