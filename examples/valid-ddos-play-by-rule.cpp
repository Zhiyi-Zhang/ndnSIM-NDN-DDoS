...
main(int argc, char* argv[]) {
  ...
  std::string topo = "valid-interest-ddos-new";
  ...
  consumerHelper.SetAttribute("GoodConsumer", BooleanValue(false));
  consumerHelper.SetAttribute("PlayByRule", BooleanValue(true));
  ...
  ndn::L3RateTracer::InstallAll("src/ndnSIM/Results/valid-play-by-rule/" + outFile + ".txt",
                                Seconds(0.5));
  Simulator::Run();
  Simulator::Destroy();
