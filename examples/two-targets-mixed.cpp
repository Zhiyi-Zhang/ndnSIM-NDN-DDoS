...
// ./waf --run "two-targets-mixed --RngRun=2 --maxRange=400 --frequency=100 --topo=ddos-simple --output=test"
...
main(int argc, char* argv[]) {
  ...
  std::string capacity = "1000";
  std::string tolerance = "500";
  ...
  Ptr<Node> as1_cs_backend = Names::Find<Node>("as1-cs-backend");
  ...
  producerHelper.SetPrefix("/u2");
  producerHelper.Install(as1_cs_backend);
  ...
  ndn::L3RateTracer::InstallAll("src/ndnSIM/Results/two-targets-mixed/" + outFile + ".txt",
                                Seconds(0.5));
  Simulator::Run();
  Simulator::Destroy();
