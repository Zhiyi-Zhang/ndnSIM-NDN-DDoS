...
labelstr = paste(c("Capacity (", as.numeric(fake_threshold), ") + Good Interest (", validAmount, ")"), collapse = "")
g.nodes <- ggplot(result) + 
  ...
  annotate("text", x=15, y=as.numeric(fake_threshold) + validAmount, vjust = -1, label = labelstr ) +
  xlab("Time [second]") +
