...
data.victim2 = subset(data, Node %in% c("as1-cs-backend"))
...
data.victim2 = summaryBy(. ~ Time + Type, data=data.victim2, FUN=sum)
...
g.nodes <- ggplot(result) + 
  ...
  geom_hline(yintercept = as.numeric(fake_threshold) + as.numeric(valid_threshold), linetype="dashed") + 
  annotate("text", x=15, y=as.numeric(fake_threshold) + as.numeric(valid_threshold), vjust = -1, label = labelstr ) +
  xlab("Time [second]") +
